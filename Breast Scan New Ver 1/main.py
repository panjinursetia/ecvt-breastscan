...
sm_dir = "F:/CHPC/Breast Scan New Ver 1/database"
class Visualization(HasTraits):
    ...
    n_min   = Range(0, 100, 1, )#mode='spinner')
    n_max    = Range(0, 100, 1, )#mode='spinner') 
    ...
    def ambil_data_scan(self):
        sm_data=h5py.File('C:/SM20CH.h5')
	v_data = h5py.File('C:/V.h5')
...
class MayaviQWidget(QtGui.QWidget):
	...
	print "test"
    def get_recons_data(self,ve,vf,vr):
	sm_data=h5py.File('C:/SM20CH.h5')
	v_data = h5py.File('C:/V.h5')
	an3d=array(sm_data['an3d'])   
	ant3d=array(sm_data['ant3d'])	
    ...
    def clearPlt(self,ve,vf,vr):
	s = self.get_recons_data(ve,vf,vr)
	self.visualization.update_plot(s)
    
    def plot3d_data(self, s):
	self.visualization.update_plot(s)
...
class Kalibrasi(MyMplCanvas):     
	...
	if calIndex == 0:
	    ...
	    self.vr = data 
	    self.axes.grid(True)
	    self.line_kalibrasi_empty, = self.axes.plot(self.vr,'g', label="Vr") 
	    self.draw() 	
...
class Kapasitansi(MyMplCanvas):     
    ...
    def update_figure(self, ve, vf, vr):		
	self.ve = ve
	self.vf = vf
	self.vr = vr
	self.axes.grid(True)
	self.line_kalibrasi_empty = self.axes.plot(self.ve,'r',self.vf,'b',self.vr,'g' )
	#self.line_kalibrasi_full, = self.axes.plot(self.vf,'b', label="Vf") 
	self.draw()  
    
    def update_figure_cal(self, ve, vf):		
	self.ve = ve
	self.vf = vf
	self.axes.grid(True)
	self.line_kalibrasi_empty = self.axes.plot(self.ve,'r',self.vf,'b')
...
class Ui_Dialog(object):
	...
	icon_dialog = QtGui.QIcon()
	icon_dialog.addPixmap(QtGui.QPixmap(_fromUtf8("icon/Picture1(1).png")), QtGui.QIcon.Normal, QtGui.QIcon.Off)
	Dialog.setWindowIcon(icon_dialog)	
        ...
        self.comboBox.addItem(_fromUtf8(""))
        self.comboBox.setItemText(2, QtGui.QApplication.translate("Dialog", "Run Capasitance", None, QtGui.QApplication.UnicodeUTF8))
        ...
        self.label_6.setText(QtGui.QApplication.translate("Dialog", "Channel", None, QtGui.QApplication.UnicodeUTF8))
	...
	#self.ports_list = QtGui.QListWidget(self.tab_3)
	
	#for portname in enumerate_serial_ports():
		    #self.ports_list.addItem(portname)
	self.comboBoxPort = QtGui.QComboBox(self.tab_3)
        self.comboBoxPort.setGeometry(QtCore.QRect(390, 178, 91, 20))
	
	i = 0
	for portname in enumerate_serial_ports():	    
	    self.comboBoxPort.addItem(_fromUtf8(portname))
	    self.comboBoxPort.setItemText(i, QtGui.QApplication.translate("Dialog", portname, None, QtGui.QApplication.UnicodeUTF8))
	    i = i+1
    ...
    def retranslateUi(self, Dialog):
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab), QtGui.QApplication.translate("Dialog", "Reconstruction", None, QtGui.QApplication.UnicodeUTF8))
